...
$old_error_handler = set_error_handler("form_asignado_a_scriptcase_error_handler");

function form_asignado_a_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['form_asignado_a']['contr_erro']) && $_SESSION['scriptcase']['form_asignado_a']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_form_asignado_a->contr_form_asignado_a->Apl_com_erro = true;
            if ($inicial_form_asignado_a->contr_form_asignado_a->NM_ajax_flag)
            {
                $inicial_form_asignado_a->contr_form_asignado_a->NM_ajax_info['errList']['geral_form_asignado_a'][] = $msg;
                return;
            }
            form_asignado_a_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function form_asignado_a_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['form_asignado_a'])  && '' != $_SESSION['scriptcase']['error_icon']['form_asignado_a'])  ? $_SESSION['scriptcase']['error_icon']['form_asignado_a']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['form_asignado_a']) && '' != $_SESSION['scriptcase']['error_close']['form_asignado_a']) ? $_SESSION['scriptcase']['error_close']['form_asignado_a'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['form_asignado_a']) && '' != $_SESSION['scriptcase']['error_span_title']['form_asignado_a']) ? $_SESSION['scriptcase']['error_span_title']['form_asignado_a'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['form_asignado_a']) && '' != $_SESSION['scriptcase']['error_icon_title']['form_asignado_a']) ? $_SESSION['scriptcase']['error_icon_title']['form_asignado_a'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['form_asignado_a']['error_buffer'] .= ob_get_flush();
}
