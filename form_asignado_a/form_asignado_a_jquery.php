...
function scEventControl_init(iSeqRow) {
  scEventControl_data["cientifico_" + iSeqRow] = {"blur": false, "change": false, "autocomp": false, "original": "", "calculated": ""};
  scEventControl_data["proyecto_" + iSeqRow] = {"blur": false, "change": false, "autocomp": false, "original": "", "calculated": ""};
}

function scEventControl_active(iSeqRow) {
  if (scEventControl_data["cientifico_" + iSeqRow]["blur"]) {
    return true;
  }
  if (scEventControl_data["cientifico_" + iSeqRow]["change"]) {
    return true;
  }
  if (scEventControl_data["proyecto_" + iSeqRow]["blur"]) {
    return true;
  }
  if (scEventControl_data["proyecto_" + iSeqRow]["change"]) {
    return true;
  }
  return false;
...
function scJQEventsAdd(iSeqRow) {
  $('#id_sc_field_cientifico_' + iSeqRow).bind('blur', function() { sc_form_asignado_a_cientifico__onblur(this, iSeqRow) })
                                         .bind('change', function() { sc_form_asignado_a_cientifico__onchange(this, iSeqRow) })
                                         .bind('focus', function() { sc_form_asignado_a_cientifico__onfocus(this, iSeqRow) });
  $('#id_sc_field_proyecto_' + iSeqRow).bind('blur', function() { sc_form_asignado_a_proyecto__onblur(this, iSeqRow) })
                                       .bind('change', function() { sc_form_asignado_a_proyecto__onchange(this, iSeqRow) })
                                       .bind('focus', function() { sc_form_asignado_a_proyecto__onfocus(this, iSeqRow) });
} // scJQEventsAdd

function sc_form_asignado_a_cientifico__onblur(oThis, iSeqRow) {
  do_ajax_form_asignado_a_validate_cientifico_(iSeqRow);
  scCssBlur(oThis, iSeqRow);
}

function sc_form_asignado_a_cientifico__onchange(oThis, iSeqRow) {
  nm_check_insert(iSeqRow);
}

function sc_form_asignado_a_cientifico__onfocus(oThis, iSeqRow) {
  scEventControl_onFocus(oThis, iSeqRow);
  scCssFocus(oThis, iSeqRow);
}

function sc_form_asignado_a_proyecto__onblur(oThis, iSeqRow) {
  do_ajax_form_asignado_a_validate_proyecto_(iSeqRow);
  scCssBlur(oThis, iSeqRow);
}

function sc_form_asignado_a_proyecto__onchange(oThis, iSeqRow) {
  nm_check_insert(iSeqRow);
}

function sc_form_asignado_a_proyecto__onfocus(oThis, iSeqRow) {
  scEventControl_onFocus(oThis, iSeqRow);
  scCssFocus(oThis, iSeqRow);
}
...
function displayChange_block_0(status) {
	displayChange_field("cientifico_", "", status);
	displayChange_field("proyecto_", "", status);
}

function displayChange_row(row, status) {
	displayChange_field_cientifico_(row, status);
	displayChange_field_proyecto_(row, status);
}

function displayChange_field(field, row, status) {
	if ("cientifico_" == field) {
		displayChange_field_cientifico_(row, status);
	}
	if ("proyecto_" == field) {
		displayChange_field_proyecto_(row, status);
...
function displayChange_field_cientifico_(row, status) {
}

function displayChange_field_proyecto_(row, status) {
}
...
function scHidePage(pageNo) {
	$("#id_form_asignado_a_form" + pageNo).hide();
}
