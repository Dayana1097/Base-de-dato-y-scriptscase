...
function scEventControl_init(iSeqRow) {
  ...
  scEventControl_data["nomapls_" + iSeqRow] = {"blur": false, "change": false, "autocomp": false, "original": "", "calculated": ""};
}
...
function scEventControl_active(iSeqRow) {
  ...
  if (scEventControl_data["nomapls_" + iSeqRow]["blur"]) {
    return true;
  }
  if (scEventControl_data["nomapls_" + iSeqRow]["change"]) {
    return true;
  }
  return false;
...
function scJQEventsAdd(iSeqRow) {
  $('#id_sc_field_dni_' + iSeqRow).bind('blur', function() { sc_form_cientificos_dni__onblur(this, iSeqRow) })
                                  .bind('change', function() { sc_form_cientificos_dni__onchange(this, iSeqRow) })
                                  .bind('focus', function() { sc_form_cientificos_dni__onfocus(this, iSeqRow) });
  $('#id_sc_field_nomapls_' + iSeqRow).bind('blur', function() { sc_form_cientificos_nomapls__onblur(this, iSeqRow) })
                                      .bind('change', function() { sc_form_cientificos_nomapls__onchange(this, iSeqRow) })
                                      .bind('focus', function() { sc_form_cientificos_nomapls__onfocus(this, iSeqRow) });
} // scJQEventsAdd

function sc_form_cientificos_dni__onblur(oThis, iSeqRow) {
  do_ajax_form_cientificos_validate_dni_(iSeqRow);
  scCssBlur(oThis, iSeqRow);
}

function sc_form_cientificos_dni__onchange(oThis, iSeqRow) {
  nm_check_insert(iSeqRow);
}

function sc_form_cientificos_dni__onfocus(oThis, iSeqRow) {
  scEventControl_onFocus(oThis, iSeqRow);
  scCssFocus(oThis, iSeqRow);
}

function sc_form_cientificos_nomapls__onblur(oThis, iSeqRow) {
  do_ajax_form_cientificos_validate_nomapls_(iSeqRow);
  scCssBlur(oThis, iSeqRow);
}

function sc_form_cientificos_nomapls__onchange(oThis, iSeqRow) {
  nm_check_insert(iSeqRow);
}

function sc_form_cientificos_nomapls__onfocus(oThis, iSeqRow) {
  scEventControl_onFocus(oThis, iSeqRow);
  scCssFocus(oThis, iSeqRow);
}
...
function displayChange_block_0(status) {
	...
	displayChange_field("nomapls_", "", status);
}

function displayChange_row(row, status) {
	displayChange_field_dni_(row, status);
	displayChange_field_nomapls_(row, status);
}
...
function displayChange_field_nomapls_(row, status) {
}
...
function scHidePage(pageNo) {
	$("#id_form_cientificos_form" + pageNo).hide();
}
