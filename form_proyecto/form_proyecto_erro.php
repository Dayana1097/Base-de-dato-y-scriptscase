...
$old_error_handler = set_error_handler("form_proyecto_scriptcase_error_handler");

function form_proyecto_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['form_proyecto']['contr_erro']) && $_SESSION['scriptcase']['form_proyecto']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_form_proyecto->contr_form_proyecto->Apl_com_erro = true;
            if ($inicial_form_proyecto->contr_form_proyecto->NM_ajax_flag)
            {
                $inicial_form_proyecto->contr_form_proyecto->NM_ajax_info['errList']['geral_form_proyecto'][] = $msg;
                return;
            }
            form_proyecto_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function form_proyecto_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['form_proyecto'])  && '' != $_SESSION['scriptcase']['error_icon']['form_proyecto'])  ? $_SESSION['scriptcase']['error_icon']['form_proyecto']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['form_proyecto']) && '' != $_SESSION['scriptcase']['error_close']['form_proyecto']) ? $_SESSION['scriptcase']['error_close']['form_proyecto'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['form_proyecto']) && '' != $_SESSION['scriptcase']['error_span_title']['form_proyecto']) ? $_SESSION['scriptcase']['error_span_title']['form_proyecto'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['form_proyecto']) && '' != $_SESSION['scriptcase']['error_icon_title']['form_proyecto']) ? $_SESSION['scriptcase']['error_icon_title']['form_proyecto'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['form_proyecto']['error_buffer'] .= ob_get_flush();
}
